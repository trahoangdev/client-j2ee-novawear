...
export function ProductCard({ product, className }: ProductCardProps) {
  ...
  return (
    <article
      className={cn(
        'group relative bg-card p-4 sm:p-5 rounded-[2rem] border border-border/40',
        'transition-all duration-500 ease-out hover:shadow-2xl hover:border-border/80 hover:-translate-y-1 block',
        className
      )}
    ...
    >
      ...
      <Link to={`/product/${slug}`} className="block relative aspect-[4/3] rounded-2xl overflow-hidden bg-muted/20 mb-5 focus:outline-none">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
          ...
          {bestseller && (
            <Badge className="bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold px-3 py-1 border-none shadow-sm rounded-full">Hot</Badge>
          )}
        </div>
      ...
      <div className="flex justify-between items-center mb-3">
        <Badge className="bg-primary text-primary-foreground hover:bg-primary/90 border-none rounded-full px-3 py-0.5 text-xs font-semibold">
          {categoryName}
        </Badge>
        <div className="flex items-center gap-1.5 bg-muted/50 px-2 py-0.5 rounded-full">
          <Star className="h-3.5 w-3.5 fill-foreground text-foreground -mt-0.5" aria-hidden />
          <span className="text-xs font-bold">{parseFloat(rating.toString()).toFixed(1)}</span>
      ...
      <Link to={`/product/${slug}`} className="block focus:outline-none">
        <h3 className="font-bold text-lg leading-snug line-clamp-2 hover:text-primary transition-colors mb-3">
          {product.name}
        </h3>
      </Link>
      ...
      <div className="flex gap-3 mt-auto">
        <Button 
          className="flex-1 rounded-2xl h-12 bg-primary text-primary-foreground font-bold text-[15px] hover:bg-primary/90 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5" 
          asChild
        >
        ...
        <Button 
          variant="outline" 
          className="h-12 w-12 rounded-2xl border-2 border-border/80 hover:border-foreground/40 hover:bg-muted text-foreground transition-all p-0 flex items-center justify-center shrink-0 hover:-translate-y-0.5" 
          asChild
        >
